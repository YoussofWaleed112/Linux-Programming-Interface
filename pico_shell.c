...
static int execute(char **args, int count) {
    ...
    if      (strcmp(args[0], "exit") == 0) { printf("Good Bye\n"); exit(0); }
    else if (strcmp(args[0], "echo") == 0)  return cmd_echo(args, count);
    else if (strcmp(args[0], "pwd")  == 0)  return cmd_pwd();
    else if (strcmp(args[0], "cd")   == 0)  return cmd_cd(args, count);
    else                                    return run_external(args);
}
