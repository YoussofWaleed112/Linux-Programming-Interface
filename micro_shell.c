...
void destroy_command(shell_command_t *cmd) {
    if (cmd) {
...
int parse_input_line(char *line, size_t size, shell_command_t *out) {
...
variable_node_t *create_variable(const char *key, const char *val) {
    variable_node_t *node = (variable_node_t *)malloc(sizeof(variable_node_t));
    if (node) {
...
void set_variable(variable_node_t **head, const char *key, const char *val) {
    if (!head || !key) return;
    
    variable_node_t *curr = *head;
    variable_node_t *prev = NULL;
    ...
    variable_node_t *new_node = create_variable(key, val);
    if (prev) {
        prev->next = new_node;
    } else {
        *head = new_node;
...
const char *get_variable(variable_node_t *head, const char *key) {
    while (head) {
...
void clear_variables(variable_node_t **head) {
    if (!head || !*head) return;
    
    variable_node_t *curr = *head;
    while (curr) {
        variable_node_t *next = curr->next;
        free(curr->key);
        free(curr->val);
        free(curr);
        curr = next;
...
bool process_assignment(shell_context_t *shell, shell_command_t *cmd) {
    ...
    *equal = '\0';
    ...
    set_variable(&shell->local_vars, key, value);
    return true;
}

char *expand_variables(shell_context_t *shell, const char *str) {
    if (!str) return NULL;
    ...
    for (size_t i = 0; str[i] != '\0';) {
        if (str[i] == '$') {
            ...
            if (var_len > 0) {
                ...
                const char *val = get_variable(shell->local_vars, varname);
                if (val) {
...
void expand_command_args(shell_context_t *shell, shell_command_t *cmd) {
    for (int i = 0; i < cmd->argc; ++i) {
        char *new_val = expand_variables(shell, cmd->argv[i]);
        free(cmd->argv[i]);
        cmd->argv[i] = new_val;
    }
}

int apply_redirections(shell_context_t *shell, shell_command_t *cmd) {
    int ret_flags = 0;
    ...
    for (int i = 0; i < cmd->argc; ++i) {
        ...
        if ((is_in || is_out || is_err) && i + 1 < cmd->argc) {
            ...
            char *filename = expand_variables(shell, cmd->argv[i + 1]);
...
void restore_descriptors(shell_context_t *shell, int flags) {
...
static int execute_builtin(shell_context_t *shell, shell_command_t *cmd) {
...
static void execute_binary(shell_context_t *shell, shell_command_t *cmd) {
    pid_t pid = fork();
...
int execute_command(shell_context_t *shell, shell_command_t *cmd) {
    if (process_assignment(shell, cmd)) return 0;
    
    expand_command_args(shell, cmd);
    if (cmd->argc == 0) return 0;
    
    int redir_flags = apply_redirections(shell, cmd);
    if (redir_flags == -1) {
        shell->status = 1;
        return 0;
    }
    ...
    restore_descriptors(shell, redir_flags);
    return 0;
}
...
int microshell_main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (parse_input_line(input, strlen(input), &cmd) == 0) {
            if (execute_command(&shell, &cmd) == 1) break;
        } else {
    ...
    free(input);
    destroy_command(&cmd);
    clear_variables(&shell.local_vars);
